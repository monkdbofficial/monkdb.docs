...
| Discovery | SELECT * FROM pg_catalog.pg_proc WHERE proname = 'extract_second'; |

## Purpose

Executes the scalar function extract second as registered in the MonkDB scalar function registry.

## Syntax

~~~sql
SELECT extract_second(current_timestamp);
~~~
