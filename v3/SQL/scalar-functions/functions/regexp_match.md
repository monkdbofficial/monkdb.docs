...
| Discovery | SELECT * FROM pg_catalog.pg_proc WHERE proname = 'regexp_match'; |

## Purpose

Executes the scalar function regexp match as registered in the MonkDB scalar function registry.

## Syntax

~~~sql
SELECT regexp_match('foo=42', '([0-9]+)');
~~~
