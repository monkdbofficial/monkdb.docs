...
| Discovery | SELECT * FROM pg_catalog.pg_proc WHERE proname = 'extract_hour'; |

## Purpose

Executes the scalar function extract hour as registered in the MonkDB scalar function registry.

## Syntax

~~~sql
SELECT extract_hour(current_timestamp);
~~~
