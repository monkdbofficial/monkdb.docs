...
| Discovery | SELECT * FROM pg_catalog.pg_proc WHERE proname = 'extract_year'; |

## Purpose

Executes the scalar function extract year as registered in the MonkDB scalar function registry.

## Syntax

~~~sql
SELECT extract_year(current_timestamp);
~~~
