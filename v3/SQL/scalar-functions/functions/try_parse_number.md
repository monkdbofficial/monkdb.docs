...
| Discovery | SELECT * FROM pg_catalog.pg_proc WHERE proname = 'try_parse_number'; |

## Purpose

Executes the scalar function try parse number as registered in the MonkDB scalar function registry.

## Syntax

~~~sql
SELECT try_parse_number(...);
~~~
