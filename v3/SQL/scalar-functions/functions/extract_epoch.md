...
| Discovery | SELECT * FROM pg_catalog.pg_proc WHERE proname = 'extract_epoch'; |

## Purpose

Executes the scalar function extract epoch as registered in the MonkDB scalar function registry.

## Syntax

~~~sql
SELECT extract_epoch(current_timestamp);
~~~
