...
| Field | Value |
| --- | --- |
| Function | try_parse_timestamp |
| Category | Other |
| Scope | Scalar expression |
| Discovery | SELECT * FROM pg_catalog.pg_proc WHERE proname = 'try_parse_timestamp'; |

## Purpose

Executes the scalar function try parse timestamp as registered in the MonkDB scalar function registry.

## Syntax

~~~sql
SELECT try_parse_timestamp(...);
~~~
