...
| Discovery | SELECT * FROM pg_catalog.pg_proc WHERE proname = 'graph_neighbors'; |

## Purpose

Returns graph metadata or traversal results from MonkDB graph schema helpers.

## Syntax

~~~sql
SELECT graph_neighbors(...);
~~~
