...
| Discovery | SELECT * FROM pg_catalog.pg_proc WHERE proname = 'extract_minute'; |

## Purpose

Executes the scalar function extract minute as registered in the MonkDB scalar function registry.

## Syntax

~~~sql
SELECT extract_minute(current_timestamp);
~~~
