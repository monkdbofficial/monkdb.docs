...
| Discovery | SELECT * FROM pg_catalog.pg_proc WHERE proname = 'parse_uri'; |

## Purpose

Parses URI or URL text and returns structured object fields for host/path/query inspection.

## Syntax

~~~sql
SELECT parse_uri('https://example.com/path?q=1');
~~~
