...
## Detailed Reference
Removes a graph namespace from graph metadata.
...
- `graph_name`: Graph identifier to remove.
...
- Drop or detach graph-bound vertex/edge metadata first when dependency checks apply.
- Prefer `IF EXISTS` in CI/CD teardown scripts.
