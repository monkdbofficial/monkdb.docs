...
```sql
DROP VERTEX TABLE [IF EXISTS] schema.table
```
...
## Detailed Reference
Removes graph vertex registration for a table.
...
```sql
DROP VERTEX TABLE IF EXISTS doc.users;
```

## Notes

- This removes graph metadata binding, not the underlying relational table.
- Use regular `DROP TABLE` separately if you also need to delete table storage.
