# Docker Compose (2-nodes)

This guide maps directly to `docker-compose.2node.yml` in this repository.

## Topology

- `monkdb001`: HTTP `6000`, PGWire `5432`, transport `6100`
- `monkdb002`: HTTP `6001`, PGWire `5433`, transport `6101`

All nodes share cluster name `monkdb-lab` and join via `discovery.seed_hosts=monkdb001`.

## Start

```bash
docker network create monkdb || true
docker compose -f docker-compose.2node.yml up -d monkdb001 monkdb002
```

`docker-compose.2node.yml` uses an external network named `monkdb`, so `docker network create monkdb` is required before first startup.
...
services:
  monkdb001:
    image: rg.fr-par.scw.cloud/monkdb/monkdb:26.3.2-amd64
...
```bash
docker compose -f docker-compose.2node.yml down
```

## Reset data volumes

```bash
docker compose -f docker-compose.2node.yml down -v --remove-orphans
